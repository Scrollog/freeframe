...
DEFAULT_INSTANCE_ORG_NAME = "FreeFrame"


class InstanceBranding(Base):
    """Singleton-backed, deployment-wide white-label settings.

    Asset keys deliberately live separately from :class:`InstanceSettings`:
    branding must be readable by unauthenticated login and public-share pages,
    while operational instance settings must remain authenticated-only.
    """
    ...
    org_name: Mapped[str] = mapped_column(
        String(255), nullable=False, default=DEFAULT_INSTANCE_ORG_NAME, server_default=DEFAULT_INSTANCE_ORG_NAME
    )
    ...
    powered_by_freeframe: Mapped[bool] = mapped_column(
